...
public class FontPreference extends DialogPreference implements DialogInterface.OnClickListener
{
    ...
    protected void onPrepareDialogBuilder( Builder builder )
    {
        ...
        HashMap< String, String > fonts = fontManager.getFonts("/sdcard/fonts", true); //$NON-NLS-1$
        m_fontPaths = new ArrayList< String >();
        m_fontNames = new ArrayList< String >();
 
        // Get the current value to find the checked item
        
        if ( this.isPersistent() && TextUtils.isEmpty(this.selectedFontPath) ) 
        {
            this.selectedFontPath = getSharedPreferences().getString( getKey(), ""); //$NON-NLS-1$
        }
    ...
    public void onClick(DialogInterface dialog, int which)
    {
        ...
        {
            this.selectedFontPath = m_fontPaths.get(which);
            
            boolean doSet = this.callChangeListener(selectedFontPath);
            ...
            if ( doSet ) 
            {
                if ( this.isPersistent() ) 
                {
                    ...
                    editor.putString( getKey(), this.selectedFontPath );
                    editor.commit();
                }
            }
            dialog.dismiss();
    ...
    {
        String font = fontManager.getFont(path);
        if ( font != null ) this.selectedFontPath = path;
    }
}