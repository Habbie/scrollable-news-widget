...
import fr.gdi.android.news.preference.color.ColorPanelView;
import fr.gdi.android.news.preference.color.ColorPickerView;

public class ColorPickerActivity extends Activity implements View.OnClickListener
{
    
    public final static String INTENT_DATA_INITIAL_COLOR = "color"; //$NON-NLS-1$
    public final static String INTENT_DATA_ALPHA_SLIDER = "alpha"; //$NON-NLS-1$
    
    public final static String RESULT_COLOR = "color"; //$NON-NLS-1$
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        boolean alphaSlider = false;
        
        if (b != null)
        {
            initialColor = b.getInt(INTENT_DATA_INITIAL_COLOR);
            alphaSlider = b.getBoolean(INTENT_DATA_ALPHA_SLIDER, false);
        }
        
        setUp(initialColor, alphaSlider);
    ...
    private void setUp(int color, boolean alphaSlider)
    {
        ...
        ((LinearLayout) mOldColorPanel.getParent()).setPadding(Math.round(mColorPickerView.getDrawingOffset()), 0,
